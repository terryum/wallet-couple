...
* **범위:** 현대카드, 롯데카드, 삼성카드, KB국민카드, 우리은행, 온누리상품권, 성남사랑상품권
* **최종 업데이트:** 2025-12-31
* **관련 문서:** [`PRD_wallet.md`](./PRD_wallet.md) (상위), [`CATEGORIES.md`](./CATEGORIES.md)
...
### 4.7. 우리은행 (Woori Bank) - 소득+지출 통합

#### 식별 키워드
* "거래내역조회", "거래일시", "찾으신금액", "맡기신금액"

#### 파일명 인식
* 파일명에 "거래내역" 포함 + `.xls` 확장자

#### 컬럼 인덱스 (헤더 기준)
| Index | 컬럼명 | 설명 |
|-------|--------|------|
| 0 | No. | 행 번호 |
| 1 | 거래일시 | 날짜+시간 (2025.12.31 08:39) |
| 2 | 적요 | 거래 유형 (CD출금, 타행이체 등) |
| 3 | 기재내용 | 이용처/송금인 |
| 4 | 찾으신금액 | 출금 금액 (지출) |
| 5 | 맡기신금액 | 입금 금액 (소득) |
| 6 | 거래후 잔액 | 잔액 |
...
#### 거래 유형 분류
* **소득 (income):** 맡기신금액 > 0, 찾으신금액 = 0
* **지출 (expense):** 찾으신금액 > 0, 맡기신금액 = 0
...
#### CD 거래 처리
* **조건:** 적요(summary)에 "CD" 포함
* **변환:** 이용처를 "ATM 인출"로 자동 설정
* **예시:** "CD출금", "CD입금" → "ATM 인출"
...
#### 데이터 검증
* 별도 합계 검증 없음 (개별 거래 기반)

### 4.8. 일반 파서 (General Parser - Fallback)
...
### 2025-12-31: 우리은행 로더 추가
- 소득/지출 통합 처리 (transaction_type 구분)
- CD 거래 → "ATM 인출" 자동 변환
- 세금, 대출이자 카테고리 자동 매핑
- 5,000원 이하 소액 거래 자동 무시
- 본인/배우자 이체, 카드결제 자동 제외

### 2025-12-30: 암호화 파일 지원
