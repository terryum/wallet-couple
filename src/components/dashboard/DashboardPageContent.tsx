...
interface DashboardPageContentProps {
  ...
  /** 모달 ID 접두사 */
  modalIdBase: string;
  /** 카테고리 클릭 시 이동할 경로 ('/' 또는 '/income') */
  categoryClickPath: string;
  /** 청구금액 비교 카드 표시 여부 */
  showBillingComparison?: boolean;
}
...
export function DashboardPageContent({
  ...
  const { data: monthlyData, isLoading: isLoadingMonthly } =
    useMonthlyAggregation(selectedMonth, selectedOwner || undefined, transactionType);

  // 추세 데이터 (12개월)
  const { data: trendData, isLoading: isLoadingTrend } =
    useMultiMonthAggregation(12, selectedOwner || undefined, transactionType);

  // 카테고리 클릭 시 내역 페이지로 이동
  const handleCategoryClick = (category: Category) => {
    router.push(`${categoryClickPath}?month=${selectedMonth}&category=${category}`);
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-40">
      ...
      <div className="px-5 pt-5">
        <div className="max-w-lg mx-auto space-y-6">
          ...
          {/* 청구금액 비교 (지출 분석에서만 표시) */}
          {showBillingComparison && <BillingComparisonCard />}
        </div>
      </div>
